...
async def generate_book_audio(
    ...
    if book.extracted_text:
        ...
        if not text:
             text = "Sorry, we could not extract text from this book due to file corruption or formatting."
        book.extracted_text = text
        db.commit()
    # Check if user has purchased the book
    is_purchased = db.query(Payment).filter(
        Payment.user_id == current_user.id,
        Payment.book_id == book_id,
        Payment.status == "success"
    ).first() is not None

    # Determine text to synthesize (truncate if not purchased)
    source_text = text if is_purchased else text[:5000]
    ...
    # Regeneration logic: if file exists but is truncated and user now has premium
    should_regenerate = False
    if os.path.exists(audio_path) and os.path.exists(alignment_path):
         try:
             with open(alignment_path, "r") as f:
                 data = json.load(f)
                 # If user has purchased but the synthesized data is short (approx pre-purchase preview limit)
                 # we regenerate to ensure they get the full content.
                 expected_tokens = len(text.strip().split())
                 if is_purchased and expected_tokens > 800 and len(data) < (expected_tokens * 0.85):
    ...
    background_tasks.add_task(generate_audio_from_text, source_text, audio_path, request.voice)
    ...
    return {"message": "Audio generation started in background", "audio_url": f"/api/v1/books/{book_id}/audio?voice={request.voice}"}
...
def get_similar_books(
    ...
    """Returns content-based similar books using Jaccard similarity on keywords."""
    target_book = db.query(Book).filter(Book.id == book_id).first()
    if not target_book:
        raise HTTPException(status_code=404, detail="Book not found")

    # Get target keywords from extracted text or genre
    target_text = target_book.extracted_text or ""
    target_genre = target_book.genre or ""
    # Combine genre tokens as extra keywords for genre-based matching
    genre_tokens = [g.strip().lower() for g in target_genre.split("/") if g.strip()]
    target_keywords = NLPService.extract_keywords(target_text, top_n=30) + genre_tokens

    # Get all other books
    all_books = db.query(Book).filter(Book.id != book_id).all()

    # Build (book_id, keywords) list
    books_keywords = []
    for b in all_books:
        b_text = b.extracted_text or ""
        b_genre = b.genre or ""
        b_genre_tokens = [g.strip().lower() for g in b_genre.split("/") if g.strip()]
        b_kws = NLPService.extract_keywords(b_text, top_n=30) + b_genre_tokens
        books_keywords.append((b.id, b_kws))

    ranked_ids = NLPService.rank_similar_books(target_keywords, books_keywords, top_n=limit)

    # Preserve ranking order
    id_to_book = {b.id: b for b in all_books}
    return [id_to_book[bid] for bid in ranked_ids if bid in id_to_book]
